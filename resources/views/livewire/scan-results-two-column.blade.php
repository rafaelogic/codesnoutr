...
<div class="h-screen flex flex-col" id="codesnoutr-scan-results-container" wire:key="scan-results-{{ $scanId }}">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
                <div class="flex items-center space-x-4">
                    ...
                    <span class="text-gray-300 dark:text-gray-600">/</span>
                    <a href="{{ route('codesnoutr.results.scan.by-issues', ['scan' => $scanId]) }}" 
    ...
    <div class="flex-1 flex overflow-scroll">
        ...
        <div class="{{$paginatedDirectoryTree && count($paginatedDirectoryTree) > 0 ? 'w-80' : 'w-[30rem]' }}  min-h-screen h-full flex-shrink-0 bg-gray-100 dark:bg-gray-600 border-r border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col">
            <!-- File Tree Header -->
                                                                        ...
                                                                        <div class="flex items-center justify-between">
                                                                            ...
                                                                            <div class="flex items-center space-x-2">
                                                                                <button wire:click="applyAutoFix({{ $instance['id'] }})" 
                                                                                        wire:loading.attr="disabled"
                                                                                        wire:target="applyAutoFix({{ $instance['id'] }})"
                                                                                        class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-white bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 disabled:from-gray-400 disabled:to-gray-500 border border-transparent rounded-lg shadow-sm transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
                                                                                    <svg class="w-3 h-3 mr-1 animate-spin hidden" wire:loading.class.remove="hidden" wire:target="applyAutoFix({{ $instance['id'] }})" fill="none" viewBox="0 0 24 24">
                                                                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                                                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                                                                    </svg>
                                                                                    <svg class="w-3 h-3 mr-1" wire:loading.remove wire:target="applyAutoFix({{ $instance['id'] }})" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                                                    </svg>
                                                                                    <span wire:loading.remove wire:target="applyAutoFix({{ $instance['id'] }})">Apply Fix</span>
                                                                                    <span wire:loading wire:target="applyAutoFix({{ $instance['id'] }})">Applying...</span>
                                                                                </button>
                                                        ...
                                                        <div class="flex items-start space-x-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div class="mt-2 flex gap-2">
                                                                    ...
                                                                    <!-- Removed API management buttons for cleaner interface -->
                                                                </div>
                                                            </div>
                                                        </div>
                                                    @endif
