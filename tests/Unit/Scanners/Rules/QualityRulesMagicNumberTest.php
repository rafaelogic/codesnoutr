...
class QualityRulesMagicNumberTest extends TestCase
{
    ...
    /** @test */
    public function it_does_not_flag_dates_and_timestamps_as_magic_numbers()
    {
        $content = '
            $date = "2024-01-01";
        ...
        $magicNumberIssues = array_filter($issues, fn($issue) => $issue['rule_id'] === 'quality.magic_number');
        $this->assertEmpty($magicNumberIssues, 'Dates and timestamps should not be flagged as magic numbers');
    }

    /** @test */
    public function it_does_not_flag_blade_html_context_as_magic_numbers()
    {
        $content = '
            @extends("layouts.app")
            ...
            @section("content")
                <img width="24" height="24" src="/icon.png">
                <div class="w-24 h-16">Utility classes</div>
            @endsection
        ';
        ...
        $magicNumberIssues = array_filter($issues, fn($issue) => $issue['rule_id'] === 'quality.magic_number');
        $this->assertEmpty($magicNumberIssues, 'Blade HTML context should not be flagged as magic numbers');
    }
    ...
    /** @test */
    public function it_allows_common_numbers()
    {
        $content = '
            <?php
            $httpOk = 200;
        ...
        $magicNumberIssues = array_filter($issues, fn($issue) => $issue['rule_id'] === 'quality.magic_number');
        $this->assertEmpty($magicNumberIssues, 'Common numbers should not be flagged');
    }
}
